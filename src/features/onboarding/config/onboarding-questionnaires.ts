import type { QuestionnaireOption, QuestionnaireStep } from "../types";

/**
 * Questionnaire option configuration
 */
...
export interface QuestionnaireStepConfig {
  stepId: string;
  stepTitle: string;
  stepOptions: QuestionnaireOptionConfig[];
  multiple: boolean;
  isSkippable: boolean;
...
/**
 * Language-specific questionnaire configuration
 */
export interface LanguageQuestionnaireConfig {
  steps: QuestionnaireStepConfig[];
}

/**
 * Main onboarding questionnaires configuration
 * Easy to configure and modify questionnaire content
 */
export const Onboarding_Questionnaires: Record<string, LanguageQuestionnaireConfig> = {
  en: {
    steps: [
      {
...
/**
 * Get questionnaire steps for a specific language
 * @param language - The language code ('en' or 'fr')
 * @returns Array of questionnaire steps for the language
 */
export const getQuestionnaireStepsByLanguage = (language: string): QuestionnaireStep[] => {
  const config = Onboarding_Questionnaires[language];
  if (!config) {
    console.warn(
      `No questionnaire configuration found for language: ${language}. Falling back to English.`
    );
    return Onboarding_Questionnaires.en.steps.map(convertConfigToStep);
  }

  return config.steps.map(convertConfigToStep);
};

/**
 * Convert configuration to QuestionnaireStep type
 * @param config - The questionnaire step configuration
 * @returns QuestionnaireStep object
 */
const convertConfigToStep = (config: QuestionnaireStepConfig): QuestionnaireStep => {
  return {
    stepId: config.stepId,
    stepTitle: config.stepTitle,
    stepOptions: config.stepOptions.map(convertOptionConfig),
    multiple: config.multiple,
    isSkippable: config.isSkippable,
    hasOtherOption: config.hasOtherOption,
    otherOptionPlaceholder: config.otherOptionPlaceholder,
  };
};

/**
 * Convert option configuration to QuestionnaireOption type
 * @param config - The option configuration
 * @returns QuestionnaireOption object
 */
const convertOptionConfig = (config: QuestionnaireOptionConfig): QuestionnaireOption => {
  return {
    id: config.id,
    label: config.label,
    value: config.value,
...
export const getAvailableLanguages = (): string[] => {
  return Object.keys(Onboarding_Questionnaires);
};
...
export const isLanguageSupported = (language: string): boolean => {
  return language in Onboarding_Questionnaires;
};
